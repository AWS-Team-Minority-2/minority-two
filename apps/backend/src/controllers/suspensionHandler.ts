...
function getMigrationDeatils(
  ...
  const migrationContent = `-- Migration ${'suspension-business'} action on table stores.store\n`;

  const migrationContentWithUpdate = (action: SuspendActionType): string => {
    let sqlContent = '';
    switch (action) {
      case 'suspend':
        sqlContent = `
  ...
  return {
    ...
    sqlContents: migrationContentWithUpdate(action),
  };
}
...
export const handleSuspendBusiness = async (suspension: SuspendAction) => {
  const hasSqlDirectory = ensureSqlExists();
  if (hasSqlDirectory) {
    const deatils = getMigrationDeatils(
      suspension.id,
      suspension.adminName,
      suspension.action
    );
    //   location of file
...
export const handleUnsuspendBusiness = async (unsuspension: SuspendAction) => {
  const hasSqlDirectory = ensureSqlExists();
  if (hasSqlDirectory) {
    const deatils = getMigrationDeatils(
      unsuspension.id,
      unsuspension.adminName,
      unsuspension.action
    );
